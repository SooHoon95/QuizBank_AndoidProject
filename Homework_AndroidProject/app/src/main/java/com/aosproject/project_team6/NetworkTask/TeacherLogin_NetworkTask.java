...
public class TeacherLogin_NetworkTask extends AsyncTask<Integer, String, Object> {
    Context context = null;
    String mAddr = null;
    ArrayList<TeacherLoginBean> teacherlogin;
    String where = null;
    // Constructor
    public TeacherLogin_NetworkTask(Context context, String mAddr, String where) {
        this.context = context;
        this.mAddr = mAddr;
        this.teacherlogin = new ArrayList<TeacherLoginBean>();
        this.where = where;
    } // constructor
    @Override
    protected Object doInBackground(Integer... integers) {
        Log.v("Message", "## doInBackgroud(TeacherLogin) ... start");
        ...
        //
        if(where.equals("select")){
            Log.v("Message", "  - doIn return workbooks : " + teacherlogin);
            return teacherlogin; // select는 엄청 많은 값이 들어올거임
        }else{
    ...
    private void parserSelect(String str){
        ...
        try{
            ...
            for(int i=0; i<jsonArray.length(); i++){
                ...
                String tpw = jsonObject1.getString("tpw");
                String tname = jsonObject1.getString("tname");
                String taddress1 = jsonObject1.getString("taddress1");
                String taddress2 = jsonObject1.getString("taddress2");
                String tdeletedate = jsonObject1.getString("tdeletedate");
                //어레이에 있는거 뽑아와서 빈
                TeacherLoginBean bean = new TeacherLoginBean(tid, tpw, tname, taddress1, taddress2, tdeletedate);
                teacherlogin.add(bean);
