...
import com.aosproject.project_team6.Bean.SubscribeBean;
...
public class Subscribe_NetworkTask extends AsyncTask<Integer, String, Object> {


    Context context = null;
    String mAddr = null;
    ArrayList<SubscribeBean> subscribe;
    String where;

    // Constructor

    public Subscribe_NetworkTask(Context context, String mAddr, String where) {
        this.context = context;
        this.mAddr = mAddr;
        this.subscribe = new ArrayList<SubscribeBean>();
        this.where = where;
    } // constructor

    @Override
    protected Object doInBackground(Integer... integers) {
        Log.v("Message", "## doInBackgroud(Subscribe) ... start");
        ...
        if(where.equals("select")){
            ...
            Log.v("Message", "  - doIn return subscribe info : " + subscribe);
            return subscribe; // select는 엄청 많은 값이 들어올거임
        }else{
    ...
    private void parserSelect(String str){
        ...
        try{
            ...
            JSONArray jsonArray = new JSONArray(jsonObject.getString("subscribe_info"));
            ...
            for(int i=0; i<jsonArray.length(); i++){
                ...
                String paydate = jsonObject1.getString("paydate");
                String teacher_id = jsonObject1.getString("teacher_id");
                //어레이에 있는거 뽑아와서 빈
                SubscribeBean bean = new SubscribeBean(paydate, teacher_id);
                subscribe.add(bean);
